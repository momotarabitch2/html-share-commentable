...
if (!values.input) throw new Error('Usage: node generate.mjs --input /absolute/report.html --output /absolute/output-dir [--title "Title"]');
...
const hscConfig = { reportId, title, spreadsheetName, maxLength: 1000 };
...
const clientConfig = JSON.stringify({ reportId, title, maxLength: 1000 }).replace(/</g, '\\u003c');
...
<h1>${escapeHtml(title)}をコメント共有する</h1><p class="lead">下の順番で、生成済みコードをGoogle Apps Scriptへ貼り付けてください。Google側の作業はChromeで行います。</p><p class="good">コメント投稿者は初回だけ表示名を入力します。表示名はそのブラウザに保存され、以降の投稿で自動的に使われます。</p><p class="note">HTMLとコメントを閲覧できる範囲はコメント機能ではなく、Apps Scriptのデプロイ画面にある「アクセスできるユーザー」で決まります。用途に合わせて組織内限定または全員を選んでください。</p>
...
<section class="card"><h2><span class="step">5</span>コメント用スプレッドシートを作る</h2><ol><li>左側で <code>Code.gs</code> をクリックします。</li><li>画面上部の「実行する関数を選択」プルダウンから <code>setupComments</code> を選びます。</li><li>その左側にある「実行」を押します。</li><li>初回の権限画面で、デプロイに使うGoogleアカウントを選びます。</li><li>「このアプリはGoogleで確認されていません」と表示されたら、いま作成した自分のプロジェクトであることを確認して「詳細」を押します。</li><li>続いて「${escapeHtml(projectName)} に移動（安全ではないページ）」を押し、権限確認画面へ進みます。</li><li>スプレッドシートへのアクセスを確認して「許可」を押します。</li><li>Apps Scriptへ戻り、画面下部の実行ログに「実行完了」と表示されるまで待ちます。Google Driveに <code>${escapeHtml(spreadsheetName)}</code> が作成され、<code>comments</code> と <code>reviewers</code> の管理シートが初期化されます。</li></ol><p class="note">この関数はApps Scriptエディタから、デプロイ所有者のアカウントで実行してください。関数一覧に出ない場合は、Code.gsの保存完了を確認して画面を再読み込みします。</p></section>
<section class="card"><h2><span class="step">6</span>ウェブアプリとして公開する</h2><ol><li>画面右上の「デプロイ」→「新しいデプロイ」を押します。</li><li>「種類を選択」の歯車アイコンを押し、「ウェブアプリ」を選びます。</li><li>「説明」は任意です。例：<code>初回公開</code></li><li>「次のユーザーとして実行」は「自分」を選びます。</li><li>「アクセスできるユーザー」で閲覧範囲を選びます。社内資料ならWorkspace組織内（例：<code>@camp-fire.jp</code>）、社外共有ならGoogleログイン必須または全員を用途に合わせて選びます。</li><li>選択したGoogleアカウントと閲覧範囲を再確認して「デプロイ」を押します。</li><li>表示されたウェブアプリURL（末尾が <code>/exec</code>）をコピーします。</li></ol><p class="note">組織内限定などGoogleログインが必要な公開範囲では、複数のGoogleアカウントへ同時ログインしているブラウザでApps ScriptのURLが開けないことがあります。その場合は、対象Workspaceアカウントだけを使うChromeプロファイルでURLを開いてください。ログイン不要の公開範囲ではこのアカウント選択は発生しません。</p></section>
<section class="card"><h2><span class="step">7</span>公開後に1件テストする</h2><ol><li><code>/exec</code> URLを開きます。</li><li>初回だけ表示名を入力し、コメントを1件投稿します。</li><li>もう一度コメント欄を開き、表示名が保持されていることを確認します。</li><li>HTMLのコメント欄に表示名、コメント場所、本文が表示されることを確認します。</li><li>非公開の <code>comments</code> シートに、表示名、場所、対象内容、コメント、該当箇所URLが1行追加されたことを確認します。</li><li>スプレッドシートのURLからHTMLの該当箇所へ戻れることを確認します。</li></ol><p class="good">ここまで確認できたら公開完了です。</p><p class="small">表示名はブラウザ単位で保持されます。別端末、別ブラウザ、シークレットウィンドウ、ブラウザデータ削除後は再入力します。表示名は本人確認ではありません。</p></section>
<section class="card"><h2>既存の公開ページを更新するとき</h2><ol><li>既存Apps Scriptの <code>Code.gs</code> と <code>Index.html</code> を、今回生成した内容で全置換して保存します。</li><li><code>setupComments</code> を選んで「実行」し、既存の <code>comments</code> と <code>reviewers</code> シートへ不足列を追加します。</li><li>「デプロイ」→「デプロイを管理」を開きます。</li><li>対象デプロイの編集アイコンを押します。</li><li>バージョンで「新バージョン」を選び、「デプロイ」を押します。</li><li><code>/exec</code> URLで、表示名の入力・保持・投稿を再確認します。</li></ol><p class="note">既存の行と列は維持されます。過去コメントに表示名がない場合は「既存レビュアー」と表示され、新しいコメントではGoogleメールを取得・保存しません。</p></section>
</main><script>
...
console.log(JSON.stringify({ input: inputPath, output: outputDir, title, projectName, spreadsheetName, files: targetPaths }, null, 2));
